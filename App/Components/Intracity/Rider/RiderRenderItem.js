...
const RiderRenderItem = ({ item, onPress }) => {
  const { t, i18n } = useTranslation();

  function GetRideStatus(item) {
    switch (item.status) {
      case 1:
        return t("accepted");
      case 2:
          ...
          <View
            style={[
              styles.pendingArea,
              {
                ...
                borderColor:
                  item.status === 4 ? Colors.secondary : Colors.primary,
              },
            ]}
          >
            <Text
              style={[
                styles.pendingText,
                {
                  color: item.status === 4 ? Colors.secondary : Colors.primary,
                },
              ]}
            >
              {GetRideStatus(item)}
...
export default RiderRenderItem;
